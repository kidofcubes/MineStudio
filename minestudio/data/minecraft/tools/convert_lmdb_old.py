...
LastEditTime: 2025-01-10 13:39:49
FilePath: /MineStudio/minestudio/data/minecraft/tools/convert_lmdb_old.py
'''
...
from minestudio.data.minecraft.callbacks import ModalConvertionCallback, SegmentationConvertionCallback
...
class ConvertWorker:
    ...
    # def process_segment(self, eps: str, segments: List[Tuple[int, Path, Path]]) -> Tuple[List, List, float]:
    #     time_start = time.time()
    #     segmentation_convertion = SegmentationConvertionCallback()
    #     cache, keys, vals = [], [], []

    #     for ord, segment_path, action_path in segments:
    #         skip_frames = self._generate_frame_indicator(action_path)
    #         _cache = segmentation_convertion.do_convert(eps, skip_frames, segment_path)
    #         cache += _cache
