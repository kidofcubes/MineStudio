...
if __name__ == '__main__':
    ...
    env = MinecraftSim(
        ...
        callbacks=[
            ...
            InitInventoryCallback(init_inventory=[
                {'slot': 0, 'type': 'oak_planks', 'quantity': 64},
                {'slot': 1, 'type': 'iron_ore', 'quantity': 1},
    ...
    # # write_video('crafting.mp4', worker.outframes)
    # env.close()
    script = SmeltScript(env)
    done, info = script.smelting('iron_ingot', 1)
    print(done, info)
    env.close()